...
int main(int argc, char *argv[]){
	if (argc < 8){
		fprintf(stderr,"usage: ./a.out [input-file] [num of queries] [selectivity] [query workload] [update workload] [time limit] [sub-runs] [query-file] [output-file]\n");
		exit(1);
	}

	int MAXN = 600000000;
	int *arr = new int[MAXN], TLE, Q, K1, K2, ROLLV=1010000000, SUB_RUNS;
	if (!arr){ fprintf(stderr,"Cannot allocate %d integers\n", MAXN); exit(1); }
	//FILE *in = fopen(argv[1],"rb"); //suvam skd
	FILE *in = fopen(argv[1],"r");
	if (!in){ fprintf(stderr,"Error opening file %s\n",argv[1]); exit(1); }
	sscanf(argv[2],"%d",&Q);
	sscanf(argv[8], "%d", &SUB_RUNS);
	int total = 0;
	int* arrpos = arr;
	//suvam skd
	/*
	...
	*/
	...
	//skd suvam
	char dataset_size[15];
	sprintf(dataset_size, "%d", N);
	char experiment_folder[100];
	strcat(experiment_folder,"experiments/");
	strcat(experiment_folder,dataset_size);

	char data_file_path[100];
	strcpy(data_file_path, experiment_folder);
	...
	*/ 
	...
	strcpy(query_file_path, experiment_folder);
	strcat(query_file_path,"/");
	strcat(query_file_path,argv[4]);
	strcat(query_file_path,".txt");

	//FILE *query_file = fopen(query_file_path,"r");
	if (argc == 10)
		strcpy(query_file_path, argv[9]);	
	FILE *query_file = fopen(argv[9], "r");
	fprintf(stderr, "%s", argv[9]);
	
	/*
	bool query_file_exists = (query_file == NULL) ? false : true;
	query_file = !query_file_exists ? fopen(query_file_path,"w") : query_file;
	fprintf(stderr, "\n %s", query_file_path);
	fprintf(stderr, "\n %p", query_file);
	*/
	...
	for (int a,b,i=0,res, ROLL=N-1; i<Q; i++){
		...
		else
		{
    		...
    		size_t len = 0;
			ssize_t read;
			read = getline(&line, &len, query_file);
			if (strchr(line, ',') != NULL)
			{
		...
		if (b == -1) {	timing(); insert(a); update_t += timing();	
		} else if (Qret == 0){ 		timing(); res = view_query(a,b); search_t = timing();
		...
		if(i == 0 || (i+1)%SUB_RUNS == 0)
			fprintf(stderr, "\n Cumulative Runtime (%d queries),%.3lf", i+1, total_t);
