...
class Builder {
 public:
  Builder(KeyType min_key, KeyType max_key, size_t num_radix_bits = 18,
          size_t max_error = 32, int default_bucket_size = 1024)
      : min_key_(min_key),
        max_key_(max_key),
        ...
        //prev_prefix_(0),
        prev_prefix_(-1){
        //spline_points_(min_key, 1, num_shift_bits_, default_bucket_size) {
        //spline_points_(min_key, num_radix_bits, num_shift_bits_, 100) {
    ...
    radix_table_.reserve(max_prefix + 1);

    size_t temp = max_error_ << 1; //2 * max_error
    size_t overflow_buffer_size = 512 ; //static_cast<size_t>(std::ceil(std::log2(temp)));

    spline_points_ = rs::Directory(min_key, num_radix_bits, num_shift_bits_, default_bucket_size, overflow_buffer_size);  
  }
  ...
  RadixSpline<KeyType> Finalize() {
    ...
    //if (curr_num_keys_ > 0 && spline_points_.back().x != prev_key_)   //skd
      //AddKeyToSpline(prev_key_, prev_position_);      //skd

    //if (curr_num_keys_ > 0 && spline_points_.getLastKeyValuePair().first != prev_key_)
      //AddKeyToSpline(prev_key_, prev_position_);

    if (curr_num_keys_ > 0 && last_key_.x != prev_key_)
      AddKeyToSpline(prev_key_, prev_position_);

    // Maybe even size the radix based on max key right from the start
    //FinalizeRadixTable();
    //std::sort(radix_table_);
    radix_table_ = spline_points_.getAllPrefixes();

    #if 0
    for (int i = 0 ; i < radix_table_.size(); i++)
    {
      rs::Bucket<KeyType> *bucket = spline_points_.getBucket(radix_table_[i]);
      std::cout << "Prefix = " << radix_table_[i] << std::endl;
      for (int j = 0 ; j < bucket->getNumElements() ; j++)
      {
        rs::BucketEntry<KeyType> *entry = bucket->getElement(j);
        std::cout << "Entry = " << entry->val << "\n\n";
      }
    }
    #endif    
 ...
 private:
  ...
  void AddKeyToSpline(KeyType key, double position) {
    //spline_points_.push_back({key, position});    //skd
    KeyType prefix = spline_points_.insert(key, position, false);
    last_key_ = {key, position};
    //PossiblyAddKeyToRadixTable(prefix);
    //radix_table_.push_back(prefix);
  }
  ...
  void PossiblyAddKeyToSpline(KeyType key, double position) {
    ...
    // `B` in algorithm.
    //const Coord<KeyType>& last = spline_points_.back();   //skd
    //std::pair<KeyType, double> temp = spline_points_.getLastKeyValuePair();
    const Coord<KeyType> last = last_key_;  
  ...
  void PossiblyAddKeyToRadixTable(KeyType key) {
    ...
    if (curr_prefix != prev_prefix_) {
      //const uint32_t curr_index = spline_points_.size() - 1;    //skd
      const uint32_t curr_index = spline_points_.globalElementCount() - 1; 
  ...
  void PossiblyAddKeyToRadixTable(KeyType prefix) {
    const KeyType curr_prefix = prefix;
    if (curr_prefix != prev_prefix_) {
      radix_table_.push_back(curr_prefix);
      prev_prefix_ = curr_prefix;
    }
  }
  ...
  void FinalizeRadixTable() {
    ++prev_prefix_;
    //const uint32_t num_spline_points = spline_points_.size();   //skd
    const uint32_t num_spline_points = spline_points_.globalElementCount();
